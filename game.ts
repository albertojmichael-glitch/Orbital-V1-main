...
document.getElementById('btnLaunch')?.addEventListener('click', () => {
    ...
    if (agencyFunds >= cost) {
        ...
        rocket.vx = INITIAL_ORBIT_SPEED; rocket.vy = 0;
        
        assemblyScreen.style.display = "none";
        hudContainer.style.display = "block";
        uiContainer.style.display = "flex";
        gameState = "VOO";
    } else alert("Sem fundos!");
});
...
let isBurningPrograde = false; let isBurningRetrograde = false;
let isMovingNodeFwd = false; let isMovingNodeBwd = false;
...
const MOON_MASS = 350; const MOON_RADIUS = 15; const MOON_ORBIT_DISTANCE = 800; const MOON_ORBIT_SPEED = 0.0001; const MOON_SOI = 150; 
...
btnReturn.addEventListener('click', () => {
    resultScreen.style.display = "none"; assemblyScreen.style.display = "flex"; updateAssemblyUI(); gameState = "MONTAGEM";
});
...
function endMission(target: string, impactSpeed: number) {
    ...
    if (impactSpeed <= MAX_SAFE_LANDING_SPEED) {
        ...
        if (target === "LUA") { resultMessage.innerText = `Você pousou na Lua a ${impactSpeed.toFixed(2)} m/s.\nBônus: R$ 250.000`; agencyFunds += 250000; } 
        else { resultMessage.innerText = `Retorno seguro à Terra a ${impactSpeed.toFixed(2)} m/s.\nRecuperação: R$ 15.000`; agencyFunds += 15000; }
    } else {
...
function drawTrajectory() {
    ...
    if (isPaused) {
        ...
        ctx.beginPath(); ctx.arc(simX, simY, 6 / cameraZoom, 0, Math.PI * 2); ctx.fill();
...
function applyEngine(dt: number) {
    ...
    if (isPaused) {
        ...
        if (isMovingNodeFwd) maneuverTime = Math.min(3900, maneuverTime + 2);
        if (isMovingNodeBwd) maneuverTime = Math.max(0, maneuverTime - 2);
        return; 
    }
    ...
    if (isBurningPrograde) {
        rocket.vx += dirX * selectedEngine.thrust * dt; rocket.vy += dirY * selectedEngine.thrust * dt;
...
function gameLoop() {
    ...
    if (gameState === "VOO") {
        ...
        if (isPaused) planText.innerHTML = `⚙️ <strong>Planejado (ΔV):</strong> ${plannedDeltaV.toFixed(2)} m/s<br>⏱️ <strong>Tempo até Queima:</strong> +${Math.floor(maneuverTime / 10)}s`;
        ...
        for (let i = 0; i < SUBSTEPS; i++) {
            ...
            if (!isPaused) {
                ...
                if (distE <= EARTH_RADIUS) { endMission("TERRA", Math.sqrt(rocket.vx**2 + rocket.vy**2)); }

                const distM = Math.sqrt((rocket.x - currentMoonX)**2 + (rocket.y - currentMoonY)**2);
                if (distM <= MOON_RADIUS) { endMission("LUA", Math.sqrt(rocket.vx**2 + rocket.vy**2)); }
            }
        }
        
        if ((isBurningPrograde || isBurningRetrograde) && rocket.currentFuel > 0 && !isPaused) {
            ctx.fillStyle = isBurningPrograde ? "#a2d149" : "#e7471d"; ctx.globalAlpha = 0.5;
            ctx.beginPath(); ctx.arc(rocket.x, rocket.y, 7, 0, Math.PI * 2); ctx.fill(); ctx.globalAlpha = 1;
        }
