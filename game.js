...
const startBurnPrograde = (e) => { if (e)
    e.preventDefault(); isBurningPrograde = true; };
const stopBurnPrograde = (e) => { if (e)
    e.preventDefault(); isBurningPrograde = false; };
const startBurnRetrograde = (e) => { if (e)
    e.preventDefault(); isBurningRetrograde = true; };
const stopBurnRetrograde = (e) => { if (e)
    e.preventDefault(); isBurningRetrograde = false; };
btnPrograde.addEventListener('mousedown', startBurnPrograde);
btnPrograde.addEventListener('mouseup', stopBurnPrograde);
btnPrograde.addEventListener('mouseleave', stopBurnPrograde);
btnRetrograde.addEventListener('mousedown', startBurnRetrograde);
btnRetrograde.addEventListener('mouseup', stopBurnRetrograde);
btnRetrograde.addEventListener('mouseleave', stopBurnRetrograde);
// ============================================================
...
function applyEngine(dt) {
    if (rocket.currentFuel <= 0 || (!isBurningPrograde && !isBurningRetrograde))
        return;
    ...
    if (isBurningPrograde) {
        rocket.vx += dirX * thrust * dt;
        rocket.vy += dirY * thrust * dt;
